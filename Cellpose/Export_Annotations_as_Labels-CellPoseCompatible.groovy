...
// USER SETTINGS
def channels_of_interest = [1,2] // null to export all the channels 
def downsample = 2 // cellpose expect your image to be scaled
...
if (training_regions.size() > 0 ) saveRegions( training_regions, channels_of_interest, downsample, 'train')

if (validation_regions.size() > 0 ) saveRegions( validation_regions, channels_of_interest, downsample, 'test')


def saveRegions( def regions, def channels, def downsample, def type ) {
    // Randomize names
    ...
    regions.eachWithIndex{ region, region_idx ->
        ...
        if  ( channels != null ){
            ...
            imp_chs = channels.collect{ c ->
                return imp_chs[c-1]   
            } as ImagePlus[]
            
            if( imp_chs.size() > 1) {
                output = RGBStackMerge.mergeChannels(imp_chs, false)
            } else {
                output = imp_chs[0]
            }
...
def saveImages(def images, def labels, def name, def type) {
    def source_folder = new File ( buildFilePath( PROJECT_BASE_DIR, 'cellpose_ground_truth', type ) )

    mkdirs( source_folder.getAbsolutePath() )
    
    println "creates : "+source_folder.getAbsolutePath()
    IJ.save( images , new File ( source_folder, name ).getAbsolutePath()+'.tif' )
    IJ.save( labels , new File ( source_folder, name ).getAbsolutePath()+'_masks.tif' )
